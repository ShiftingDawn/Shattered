...
import static dawn.lib.json.JsonValidator.createException;
import static dawn.lib.json.JsonValidator.getFieldName;
...
final class BooleanControlledFieldValidator {
	...
	static void validateRequiredByBooleanFields(final JsonObject object, final Field[] fields, @Nullable final String path) {
		final Field[] valueFields = JsonValidator.filterFields(fields, field -> field.isAnnotationPresent(Json.Required.When.class) || field.isAnnotationPresent(Json.Required.WhenNot.class));
	...
	private static Map<Field, List<Field>> collectFields(final Field[] allFields, final Field[] valueFields, final boolean expected, @Nullable final String path) {
		...
		for (final Field field : valueFields) {
			...
			final Field[] expectedFields = JsonValidator.filterFields(allFields, f -> expectedFieldName.equals(getFieldName(f)));
