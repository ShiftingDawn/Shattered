...
import dawn.internal.StringTypeValidator;
import dawn.lib.GsonHelper;
import dawn.lib.Json;
import org.jspecify.annotations.Nullable;

final class StringTypeValidatorImpl implements StringTypeValidator {

	@Override
	public void validate(final JsonObject object, final Class<?> clazz, final Object instance, @Nullable final String path) {
		try {
			...
			StringTypeValidatorImpl.validateStructure(clazz, controllers, values);
			final HashMap<String, String> controllerValues = StringTypeValidatorImpl.validateControllerValues(controllers, instance, path);
			StringTypeValidatorImpl.validateParsed(object, values, instance, controllerValues, path);
			...
			for (final Field field : clazz.getDeclaredFields()) {
				...
				//Validate nested objects
				if (GsonHelper.gson().getAdapter(fieldType) instanceof ReflectiveTypeAdapterFactory.Adapter) {
					final String jsonName = JsonValidatorImpl.getFieldName(field);
					this.validate(object.getAsJsonObject(jsonName), fieldType, field.get(instance), (path == null ? "" : path + ".") + jsonName);
				}
			}
	...
	private static void validateStructure(final Class<?> clazz, final List<Field> controllers, final List<Field> values) {
	...
	private static HashMap<String, String> validateControllerValues(final List<Field> controllers, final Object instance, @Nullable final String path) throws IllegalAccessException {
		...
		for (final Field field : controllers) {
			if (field.isAnnotationPresent(Json.TypeControllerString.class)) {
				...
				if (!Arrays.asList(annotation.accepted()).contains(value)) {
					throw JsonValidatorImpl.createException("Field has invalid value (accepts: %s): '%s'", path, Arrays.toString(annotation.accepted()), JsonValidatorImpl.getFieldName(field));
				}
				...
				if (value == null) {
					...
					throw JsonValidatorImpl.createException("Field has invalid value (accepts: %s): '%s'", path, Arrays.toString(field.getType().getEnumConstants()), JsonValidatorImpl.getFieldName(field));
				}
	...
	private static void validateParsed(final JsonObject object, final List<Field> values, final Object instance, final HashMap<String, String> controllerValues, @Nullable final String path)
		...
		for (final Field field : values) {
			...
			if (field.isAnnotationPresent(Json.Required.class) && field.getAnnotation(Json.Required.class).group().length == 0 && controllerValue.equalsIgnoreCase(annotation.type())) {
				throw JsonValidatorImpl.createException("Field(%s=%s) was marked required but controller was not defined!", path, JsonValidatorImpl.getFieldName(field), annotation.type());
			}
			...
			if (field.isAnnotationPresent(Json.Required.class) && field.getAnnotation(Json.Required.class).group().length == 0 && !field.isAnnotationPresent(Json.TypeValue.class)) {
				final String jsonName = JsonValidatorImpl.getFieldName(field);
				if (GsonHelper.isNull(object, jsonName)) {
					throw JsonValidatorImpl.createException("Missing (or invalid) required attribute '%s'", path, jsonName);
				}
			}
		}
		JsonValidatorImpl.validateRequiredGroups(object, values.stream().filter(field -> {
			if (!field.isAnnotationPresent(Json.Required.class)) {
				return false;
			}
			final Json.TypeValue annotation = field.getAnnotation(Json.TypeValue.class);
