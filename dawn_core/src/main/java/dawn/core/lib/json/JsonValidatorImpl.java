...
import dawn.internal.JsonValidator;
import dawn.lib.GsonHelper;
import dawn.lib.Json;
import org.jspecify.annotations.Nullable;

final class JsonValidatorImpl implements JsonValidator {
	...
	@Override
	public void validate(final JsonObject object, final Class<?> type, @Nullable final String path) {
		//Collect all non-ignored fields
		final Field[] allFields = JsonValidatorImpl.collectFields(type, field -> {
		...
		final Field[] requiredFields = JsonValidatorImpl.filterFields(allFields, field -> field.isAnnotationPresent(Json.Required.class) && !field.isAnnotationPresent(Json.TypeValue.class));
		for (final Field field : requiredFields) {
			final String name = JsonValidatorImpl.getFieldName(field);
			...
			if (required.group().length == 0) {
				if (GsonHelper.isNull(object, name)) {
					throw JsonValidatorImpl.createException("Missing (or invalid) required attribute '%s'", path, name);
				} else if (GsonHelper.hasString(object, name) && GsonHelper.getString(object, name).trim().isEmpty()) {
					throw JsonValidatorImpl.createException("Required string '%s' cannot be an empty string", path, name);
				}
			}
		}
		JsonValidatorImpl.validateRequiredGroups(object, requiredFields, path);
		BooleanControlledFieldValidator.validateRequiredByBooleanFields(object, allFields, path);
		final Field[] nestedClassFields = JsonValidatorImpl.filterFields(allFields, field -> GsonHelper.gson().getAdapter(field.getType()) instanceof ReflectiveTypeAdapterFactory.Adapter);
		for (final Field nested : nestedClassFields) {
			final String name = JsonValidatorImpl.getFieldName(nested);
			this.validate(object.getAsJsonObject(name), nested.getType(), (path == null ? "" : path + ".") + name);
		}
	}

	static void validateRequiredGroups(final JsonObject object, Field[] fields, @Nullable final String path) {
		fields = JsonValidatorImpl.filterFields(fields, field -> field.getAnnotation(Json.Required.class).group().length > 0);
		if (fields.length == 0) {
			return;
		}
		...
		for (final String group : groups) {
			...
			for (final Field field : JsonValidatorImpl.filterFields(fields, field -> field.getAnnotation(Json.Required.class).group()[0].groupName().equals(group))) {
		...
		for (final Map.Entry<String, Map<String, List<Field>>> entry : groupedFields.entrySet()) {
			...
			for (final Map.Entry<String, List<Field>> entry2 : entry.getValue().entrySet()) {
				for (final Field field : entry2.getValue()) {
					if (GsonHelper.isNull(object, JsonValidatorImpl.getFieldName(field))) {
						continue groupLoop;
					}
				}
				if (foundGroup == null) {
					...
					JsonValidatorImpl.throwGroupRequirementException("Only one of the following groups can be defined: ", entry.getValue(), path);
				}
			}
			if (foundGroup == null) {
				JsonValidatorImpl.throwGroupRequirementException("One of the following groups MUST be defined: ", entry.getValue(), path);
			}
		}
	}
	...
	private static void throwGroupRequirementException(final String prefix, final Map<String, List<Field>> fields, @Nullable final String path) {
		...
		for (final List<Field> allFields : fields.values()) {
			builder.append(Arrays.toString(allFields.stream().map(JsonValidatorImpl::getFieldName).toArray(String[]::new)));
			if (first) {
				builder.append(", ");
				first = false;
			}
		}
		throw JsonValidatorImpl.createException(builder.toString(), path);
	}
