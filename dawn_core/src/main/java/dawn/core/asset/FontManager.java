...
final class FontManager implements ProtoAssetProvider<ProtoFont, Font> {
	...
	private static final Logger LOGGER = LogManager.getLogger("Fonts");
	...
	void init() {
		...
		for (final ProtoFont proto : Registries.get().fonts()) {
			this.unloadFont(proto);
			this.loadFont(proto);
		}
	}

	@Override
	public Font get(final Identifier identifier) {
		return this.mapping.get(identifier);
	}

	private void loadFont(final ProtoFont proto) {
		FontManager.LOGGER.debug("\tLoading font {}", proto.getRegistryKey());
		if (this.mapping.containsKey(proto.getRegistryKey())) {
			FontManager.LOGGER.error("\tTrying to load duplicate font {}. This will most likely result in a memory leak.", proto.getRegistryKey());
		}
		final ByteBuffer fileDataBuffer;
		try {
			FontManager.LOGGER.debug("\t\tReading font file");
			final String path = this.assets.getResources().makePath(proto.getRegistryKey(), "font", "ttf");
			...
			FontManager.LOGGER.error("Could not load font {}.", proto.getRegistryKey());
		...
		this.assets.dumpAsset("font/%s.png".formatted(proto.getRegistryKey().toPathSafeString()), writer.getData());
		FontManager.LOGGER.debug("\t\tCalculating glyph data");
		final Char2ObjectArrayMap<FontImpl.GlyphImpl> glyphs = new Char2ObjectArrayMap<>();
		for (int i = 0; i < charData.capacity(); ++i) {
			final STBTTBakedChar bakedChar = charData.get(i);
			final FontImpl.GlyphImpl glyph = new FontImpl.GlyphImpl(
		...
		final Texture fontTexture = TextureManager.makeTextureFromData(new DummyProtoTexture(proto.getRegistryKey()), writer.getData(), false);
		FontManager.LOGGER.debug("\t\tCleaning up");
		writer.free();
		charData.free();
		MemoryUtil.memFree(imageData);
		MemoryUtil.memFree(bakeResult.buffer());
		final Font result = new FontImpl(proto, fontTexture, Char2ObjectMaps.unmodifiable(glyphs), lineHeight, baseline, FontManager.FONT_HEIGHT);
		this.mapping.put(proto.getRegistryKey(), result);
		FontManager.LOGGER.debug("\t\tDone");
	}

	private void unloadFont(final ProtoFont proto) {
		final Font fnt = this.mapping.get(proto.getRegistryKey());
		if (fnt != null) {
			FontManager.LOGGER.debug("\tUnloading font {} (texture={})", proto.getRegistryKey(), fnt.getTexture().id());
			glDeleteTextures(fnt.getTexture().id());
			this.mapping.remove(proto.getRegistryKey());
			FontManager.LOGGER.debug("\t\tDone");
		}
	}
